...
/*   Updated: 2023/05/25 11:34:15 by mpolisse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ClapTrap::~ClapTrap(){
	std::cout << this->_name << " \033[31mClapTrap was destroyed!\033[0m" << std::endl;
}

ClapTrap::ClapTrap(const ClapTrap &original){
	std::cout << this->_name << " \033[32mClapTrap was copied!\033[0m" << std::endl;
	*this = original;
}
...
void ClapTrap::attack(const std::string& target){
	if (this->_hitPoints <= 0){
		std::cout << this->_name << " \033[95mcan't attack because it's already destroyed.\033[0m" << std::endl;
	} else if (this->_energyPoints <= 0) {
		std::cout << this->_name << " \033[95mcan't attack because it's out of energy points.\033[0m" << std::endl;
	} else {
		std::cout <<  this->_name << " \033[94mattacks " << target << " causing " << this->_attackDamage << " points of damage!\033[0m" << std::endl;
		this->_energyPoints--;
	}
}
...
void ClapTrap::takeDamage(unsigned int amount){
	...
	if (this->_hitPoints <= 0){
		std::cout << this->_name << " \033[95mcannot be attacked because it has been destroyed\033[0m" << std::endl;
	} else {
		std::cout << this->_name << " \033[31mreceived " << amount << " damage.\033[0m" << std::endl;
